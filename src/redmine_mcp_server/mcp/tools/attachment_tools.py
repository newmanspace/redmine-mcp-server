    ...
    try:
        if DWS_AVAILABLE:
            ...
            if result and not result.get('error'):
                return result
            logger.info(f'DWS has no data for project {project_id}, falling back to API')
...
async def _summarize_from_dws(project_id: int, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        high_priority_issues = warehouse.get_high_priority_issues(project_id, today, limit=20)
        ...
        if historical_stats:
            avg_new_per_day = sum(s.get('new', 0) for s in historical_stats) / len(historical_stats)
            avg_closed_per_day = sum(s.get('closed', 0) for s in historical_stats) / len(historical_stats)
        
        by_status = current_stats.get('by_status', {})
        by_priority = current_stats.get('by_priority', {})
        
        insights = _generate_dws_insights(
            total_issues, new_today, closed_today,
            by_status, by_priority, high_priority_issues, top_assignees
        )
...
async def _summarize_from_api(project_id: int, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        created_issues = list(redmine.issue.filter(project_id=project_id, created_on=date_filter))
        updated_issues = list(redmine.issue.filter(project_id=project_id, updated_on=date_filter))
        ...
        return {
            'project': {
                ...
                'from_dws': False, 'source': 'Redmine API (fallback)',
            },
            'analysis_period': {
                'days': days,
            ...
            'project_totals': {'total_issues': len(all_issues), 'overall_breakdown': all_stats},
            'insights': {
                'daily_creation_rate': round(total_created / days, 2),
                'daily_update_rate': round(total_updated / days, 2),
                'recent_activity_percentage': round((total_updated / len(all_issues) * 100) if all_issues else 0, 2),
            },
        }
    except Exception as e:
...
def _analyze_issues(issues: List[Any]) -> Dict[str, Any]:
    ...
    return {'by_status': status_counts, 'by_priority': priority_counts, 'by_assignee': assignee_counts, 'total': len(issues)}


def _generate_dws_insights(total_issues, new_today, closed_today, by_status, by_priority, high_priority_issues, top_assignees):
    insights = {'alerts': [], 'suggestions': []}
    overdue_count = len([i for i in high_priority_issues if i.get('status_name') not in ['已关闭', '已解决']])
    if overdue_count > 5:
        insights['alerts'].append({'type': 'high_priority_overdue', 'severity': 'high', 'message': f'发现 {overdue_count} 个未关闭的高优先级 Issue', 'count': overdue_count})
    for assignee in top_assignees[:3]:
        task_count = assignee.get('total', 0)
        if task_count > 30:
            insights['alerts'].append({'type': 'workload_high', 'severity': 'medium', 'message': f"{assignee.get('assigned_to_name')} 负载过高 ({task_count} 任务)", 'assignee': assignee.get('assigned_to_name'), 'task_count': task_count})
    if new_today > 20:
    ...
    if in_progress > 50:
        insights['suggestions'].append(f'进行中的 Issue 较多 ({in_progress})，建议加快处理速度')
    return insights
