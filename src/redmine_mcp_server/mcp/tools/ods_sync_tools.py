...
async def sync_ods_incremental(
    ...
    """
    try:
        logger.info(f"MCP: Triggering incremental ODS sync for projects: {project_ids or 'all subscribed'}")
...
async def sync_ods_full(
    ...
    """
    try:
        logger.info(f"MCP: Triggering full ODS sync for projects: {project_ids or 'all subscribed'}, from_date: {from_date}")
                ...
                with conn.cursor() as cur:
                    ...
                    for table in tables:
                        ...
                        if table in ['ods_issues', 'ods_journals']:
                            date_col = 'updated_on' if table == 'ods_issues' else 'created_on'
                            cur.execute(f"SELECT MAX({date_col}) as max_date FROM warehouse.{table}")
                            result = cur.fetchone()
                            if result and result['max_date']:
                                latest_dates[table] = result['max_date'].isoformat() if hasattr(result['max_date'], 'isoformat') else str(result['max_date'])
                    ...
                    for table, max_date_str in latest_dates.items():
                        if max_date_str:
                            ...
                            if age_hours > 24:
                                status = "STALE"
                                issues.append(f"{table} data is {age_hours:.1f} hours old")
