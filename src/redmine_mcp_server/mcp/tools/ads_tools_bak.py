...
def register_ads_tools(mcp):
    ...
    async def generate_contributor_report(
        project_id: int, year_month: Optional[str] = None
    ) -> Dict[str, Any]:
        """生成贡献者分析报表"""
    ...
    async def generate_project_health_report(
        project_id: int, snapshot_date: Optional[str] = None
    ) -> Dict[str, Any]:
                ...
                with conn.cursor() as cur:
                    if not snapshot_date:
                        ...
                        snapshot_date = (
                            ...
                            else date_class.today().isoformat()
                        )
                    ...
                    health_score = min(
                        100, (closed / total * 100) if total > 0 else 100
                    )
                    risk_level = (
                        "low"
                        if health_score >= 80
                            ...
                            "medium"
                            if health_score >= 60
                            else "high" if health_score >= 40 else "critical"
                        )
                    )
    ...
    async def generate_all_ads_reports(
        project_ids: Optional[List[int]] = None, year_month: Optional[str] = None
    ) -> Dict[str, Any]:
        """生成所有 ADS 报表"""
        from .redmine_warehouse import DataWarehouse
    ...
    async def get_contributor_ranking(
        project_id: int, year_month: Optional[str] = None, limit: int = 20
    ) -> Dict[str, Any]:
        """获取贡献者排行榜"""
        from .redmine_warehouse import DataWarehouse
