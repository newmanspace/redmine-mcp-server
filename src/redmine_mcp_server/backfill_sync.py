...
class BackfillSync:
    ...
    def build_issue_timeline(self, issue: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        current_state = {
            ...
            'assigned_to_id': issue.get('assigned_to', {}).get('id') if issue.get('assigned_to') else None,
            'assigned_to_name': issue.get('assigned_to', {}).get('name') if issue.get('assigned_to') else None,
    ...
    def backfill_project(self, project_id: int, start_date: Optional[datetime] = None):
                ...
                with self.warehouse.get_connection() as conn:
                    with conn.cursor() as cur:
                        for state in snapshots:
                            ...
                            """, (
                                state['issue_id'], state['project_id'], date,
                                state['subject'], state['status_id'], state['status_name'],
                                state['priority_id'], state['priority_name'],
                                state['assigned_to_id'], state['assigned_to_name'],
                                state['created_at'], state['updated_at'],
                                state['is_new'], state['is_closed'], state['is_updated']
                            ))
        ...
        logger.info(f"Backfill completed for project {project_id}: {total_saved} snapshots saved")
