...
def get_translations(language: str = None) -> Dict[str, Any]:
    ...
    if language == 'zh_CN':
        from .zh_CN import (
            REPORT_TYPES, REPORT_LEVELS, STATUS_NAMES, PRIORITY_NAMES,
            EMAIL_SUBJECTS, EMAIL_CONTENT, SUBSCRIPTION_MESSAGES,
            DAYS_OF_WEEK, MONTHS
        )
    else:  # en_US
        from .en_US import (
...
def format_email_subject(
    ...
    if report_type == 'daily':
        ...
        return subjects['weekly'].format(project_name=project_name, date_range=date_info)
    else:  # monthly
