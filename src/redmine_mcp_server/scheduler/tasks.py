...
class RedmineSyncScheduler:
    ...
    def _load_subscribed_projects(self) -> List[int]:
        ...
        try:
            ...
            project_ids = list(set(sub.get('project_id') for sub in all_subs if sub.get('enabled', True)))
            logger.info(f"Loaded {len(project_ids)} subscribed projects: {project_ids}")
            return project_ids
    ...
    def _sync_project(self, project_id: int, incremental: bool = True, progressive: bool = False) -> int:
        """
        ...
        if progressive:
            # Progressive weekly sync: sync one week at a time from project creation
            ...
            logger.info(f"Progressive sync for project {project_id}: from {sync_start.strftime('%Y-%m-%d')}")
        
        elif incremental:
            # Incremental sync: get issues updated in last 13 minutes (with buffer to prevent missing data)
            # Use 13 minutes to ensure we don't miss any data due to timing issues or sync delays
            ...
            if project_created:
                ...
                logger.info(f"Full sync for project {project_id}, from project creation ({project_created.strftime('%Y-%m-%d')})")
            else:
        ...
        if progressive and sync_start and sync_end:
            for issue in all_issues:
                ...
                if created_on:
                    issue_date = datetime.fromisoformat(created_on.replace('Z', '+00:00')).replace(tzinfo=None)
                    if sync_start <= issue_date < sync_end:
                        filtered_issues.append(issue)
                    elif issue_date >= sync_end:
                        break  # Issues are sorted by date, can stop early
            
            logger.info(f"Filtered {len(all_issues)} issues to {len(filtered_issues)} in date range [{sync_start.strftime('%Y-%m-%d')}, {sync_end.strftime('%Y-%m-%d')})")
        ...
        try:
            self.warehouse.upsert_issues_batch(project_id, issues_to_sync, datetime.now().date(), previous_map)
            logger.info(f"Synced {len(issues_to_sync)} issues for project {project_id}")
            
            # Update progress for progressive sync
            if progressive and sync_end:
                self.project_sync_progress[project_id] = sync_end
                logger.debug(f"Project {project_id} sync progress updated to {sync_end}")
    ...
    def _sync_all_projects(self, full: bool = True, progressive: bool = False):
        ...
        logger.info(f"Starting {sync_type} sync for {len(self.project_ids)} projects...")
        
        sync_results = {}
        for project_id in self.project_ids:
            try:
                count = self._sync_project(project_id, incremental=not full, progressive=progressive)
    ...
    def _sync_all_projects_progressive(self):
        ...
        """
        logger.info(f"Starting progressive weekly sync for {len(self.project_ids)} projects...")
        
        sync_results = {}
        for project_id in self.project_ids:
            try:
                count = self._sync_project(project_id, incremental=False, progressive=True)
                sync_results[project_id] = {'status': 'success', 'count': count}
            except Exception as e:
                logger.error(f"Failed to sync project {project_id}: {e}")
                sync_results[project_id] = {'status': 'error', 'error': str(e)}
        ...
        self._sync_count += 1
        logger.info(f"Progressive sync completed (run #{self._sync_count})")
        logger.info(f"Results: {sync_results}")
    ...
    def start(self):
        ...
        try:
            ...
            def run_scheduler():
                ...
                logger.info("Note: Full/progressive sync must be triggered manually via MCP tools")
            ...
            logger.info(f"Incremental sync interval: {self.sync_interval_minutes} minutes (last 13 minutes of data)")
