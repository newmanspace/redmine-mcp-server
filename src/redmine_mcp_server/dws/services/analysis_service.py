...
class DevTestAnalyzer:
        ...
        while True:
            resp = requests.get(
                ...
                params={
                    ...
                    "status_id": self.status_resolved_id,
                    "limit": limit,
                    "offset": offset,
                },
                timeout=60,
    ...
    def _get_resolver(self, issue: Dict) -> Optional[str]:
        ...
        for journal in sorted_journals:
            ...
            for detail in details:
                if detail.get("name") == "status_id" and str(
                    detail.get("new_value", "")
                ) == str(self.status_resolved_id):
    ...
    def format_report(self, result: Dict, project_id: int) -> str:
        ...
        lines.append(
            f"Developer Team Members: {len(result.get('dev_team_members', []))}"
        )
        lines.append("")
    ...
    def extract_contributors_from_journals(
        self, journals: List[Dict], issue_id: int, project_id: int
    ) -> List[Dict]:
        """
        ...
        for journal in journals:
            ...
            contrib["last_contribution"] = journal.get(
                "created_on", contrib["last_contribution"]
            )
    ...
    def get_project_member_roles(self, project_id: int) -> List[Dict]:
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(
                    f"Failed to get project {project_id} memberships: {resp.status_code}"
                )
                return roles
            ...
            for membership in memberships:
                ...
                if user_id in seen_users:
                    ...
                    if highest_priority < existing.get("role_priority", 999):
                        existing["highest_role_id"] = (
                            highest_role.get("id") if highest_role else None
                        )
                        existing["highest_role_name"] = (
                            highest_role.get("name") if highest_role else None
                        )
