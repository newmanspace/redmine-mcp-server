...
class ReportGenerationService:
    ...
    def get_project_stats(self, project_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            open_count = sum(
                1 for i in all_issues if i.get("status", {}).get("name") != "已shutdown"
            )
            closed_count = sum(
                1 for i in all_issues if i.get("status", {}).get("name") == "已shutdown"
            )
            ...
            top_assignees = [
                ...
                for name, count in sorted(
                    assignee_count.items(), key=lambda x: x[1], reverse=True
                )
            ][:10]
    ...
    def generate_daily_report(
        ...
        if level == "comprehensive":
            ...
            if include_trend:
                trend_data = self.trend_service.analyze_daily_trend(
                    project_id, trend_days
                )
    ...
    def generate_weekly_report(
        ...
        if level == "comprehensive" and include_trend:
            trend_data = self.trend_service.analyze_weekly_trend(
                project_id, trend_weeks
            )
            report["trend_analysis"] = trend_data
    ...
    def generate_monthly_report(
        ...
        if level in ["detailed", "comprehensive"]:
            ...
            if level == "comprehensive":
                ...
                # 计算平均解决time（如果有data）
                report["avg_resolution_days"] = self._calculate_avg_resolution_time(
                    all_issues
                )

        if level == "comprehensive" and include_trend:
            trend_data = self.trend_service.analyze_monthly_trend(
                project_id, trend_months
            )
            report["trend_analysis"] = trend_data
