...
class SubscriptionReporter:
        ...
        try:
            ...
            new_issues = list(
                redmine.issue.filter(
                    project_id=project_id, created_on=f">={today}", limit=10
                )
            )
            ...
            high_priority = list(
                redmine.issue.filter(
                    project_id=project_id, priority_id="1,2,3", status_id="*", limit=10
                )
            )
            ...
            return {
                ...
                "level": "brief",
                "generated_at": datetime.now().isoformat(),
                "summary": {"new_today": len(new_issues)},
                ...
                "top_issues": [
                    {
                        ...
                        "assignee": (
                            i.assigned_to.name if i.assigned_to else "Unassigned"
                        ),
    ...
    def generate_detailed_report(self, project_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            top_assignees = self.warehouse.get_top_assignees(
                project_id, today, limit=10
            )

            # Get high priority issues
            high_priority = self.warehouse.get_high_priority_issues(
            ...
            for issue in high_priority:
                try:
                    created = datetime.strptime(
                        issue.get("created_at", "")[:10], "%Y-%m-%d"
                    )
                    age = (datetime.now() - created).days
                    if age > 30 and issue.get("status_name") not in [
                        "已shutdown",
                        "已解决",
                    ]:
                        overdue_risks.append({**issue, "age_days": age})
                except:
                    pass
            ...
            return {
                ...
                "insights": self._generate_insights(
                    stats, overdue_risks, top_assignees
                ),
            }
    ...
    def _generate_detailed_from_api(self, project_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            high_priority = [
                i
                for i in all_issues
                if i.priority and i.priority.name in ["立刻", "紧急", "高"]
            ]
            ...
            return {
                ...
                "assignee_breakdown": dict(
                    sorted(by_assignee.items(), key=lambda x: x[1], reverse=True)[:10]
                ),
                "high_priority_count": len(high_priority),
    ...
    def _generate_insights(
        self, stats: Dict, overdue_risks: List, top_assignees: List
    ) -> Dict[str, Any]:
        """generateproject洞察"""
    ...
    def format_report_for_message(
        self, report: Dict[str, Any], channel: str = "dingtalk"
    ) -> str:
        """
        format化report为消息文本
        ...
        if level == "detailed":
            ...
            if "overdue_risks" in report and report["overdue_risks"]:
                ...
                for risk in report["overdue_risks"][:5]:
                    lines.append(
                        f"- #{risk.get('issue_id')} {risk.get('subject')[:30]} ({risk.get('age_days')}天)"
                    )
                lines.append("")
        ...
        if "top_issues" in report or "high_priority_issues" in report:
            ...
            for issue in issues[:10]:
                icon = (
                    "🔴"
                    if issue.get("priority_name") == "立刻"
                    else "🟠" if issue.get("priority_name") == "紧急" else "🟡"
                )
