...
class ODSSyncService:
    ...
    def sync_ods_issues_full(
        ...
        if from_date:
            ...
            try:
                ...
                if created_on:
                    from_date = datetime.fromisoformat(created_on.replace('Z', '+00:00'))
                    date_filter = f">={from_date.strftime('%Y-%m-%d')}"
    ...
    def _fetch_and_sync_issues(
        ...
        logger.info(f"Synced {synced_count}/{len(all_issues)} issues to ODS for project {project_id}")
                ...
                for issue in issues:
                    try:
                        ...
                        if 'journals' in issue:
                            self._sync_journals_to_ods(cur, issue.get('id'), issue.get('journals', []))
    ...
    def sync_ods_users(self, user_ids: Optional[List[int]] = None) -> int:
        ...
        try:
            ...
            while True:
                ...
                resp = requests.get(users_url, headers=self.headers, params=params, timeout=30)
                resp.raise_for_status()
                data = resp.json()
    ...
    try:
        ...
        if not project_ids:
            ...
            project_ids = list(set(
                sub.get('project_id') for sub in manager.list_all_subscriptions()
...
def sync_ods_full(project_ids: Optional[List[int]] = None, from_date: Optional[datetime] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for project_id in project_ids:
            try:
                fetched, synced = sync_service.sync_ods_issues_full(project_id, from_date)
                results['projects'][project_id] = {'fetched': fetched, 'synced': synced}
                results['total_fetched'] += fetched
                results['total_synced'] += synced
            except Exception as e:
