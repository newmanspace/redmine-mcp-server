...
class DataWarehouse:
    ...
    def _init_pool(self):
        ...
        try:
            ...
            logger.info(f"Connected to PostgreSQL warehouse at {
                    self.db_host}:{
                    self.db_port}")
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                for issue in issues:
                    ...
                    is_closed = (
                        issue.get("status", {}).get("name") == "已shutdown"
                        and prev.get("status_name") != "已shutdown"
                    )
                    is_updated = (
                    ...
                    self.upsert_issue(
                        issue, snapshot_date, is_new, is_closed, is_updated
                    )
    ...
    def get_project_daily_stats(
        self, project_id: int, snapshot_date: date
    ) -> Dict[str, Any]:
    ...
    def get_high_priority_issues(
        self, project_id: int, snapshot_date: date, limit: int = 20
    ) -> List[Dict]:
        """get高priority Issue"""
        with self.get_connection() as conn:
            with conn.cursor() as cur:
    ...
    def get_top_assignees(
        self, project_id: int, snapshot_date: date, limit: int = 10
    ) -> List[Dict]:
        """get人员job量 TOP N"""
        with self.get_connection() as conn:
            with conn.cursor() as cur:
    ...
    def upsert_issue_contributors(
        self, issue_id: int, project_id: int, contributors: List[Dict]
    ):
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                for contrib in contributors:
                    ...
                    """,
                        (
                            ...
                            project_id,
                            contrib["user_id"],
                            contrib.get("user_name", ""),
                            contrib.get("highest_role_id"),
                            contrib.get("highest_role_name"),
                            contrib.get("role_category"),
                            contrib.get("journal_count", 0),
                            contrib.get("first_contribution"),
                            contrib.get("last_contribution"),
                            contrib.get("status_change_count", 0),
                            contrib.get("note_count", 0),
                ...
                # Refresh summary
                cur.execute(
                    "SELECT warehouse.refresh_dws_issue_contributor_summary(%s, %s)",
                    (issue_id, project_id),
                )
                ...
                for role in roles:
                    ...
                    """,
                        (
                            project_id,
                            role["user_id"],
                            role.get("highest_role_id"),
                            role.get("highest_role_name"),
                            role.get("role_category"),
                            role.get("role_priority"),
                            role.get("all_role_ids"),
    ...
    def refresh_dws_project_role_distribution(
        self, project_id: int, snapshot_date: date
    ):
        """刷新project角色分布"""
        with self.get_connection() as conn:
    ...
    def get_project_role_distribution(
        self, project_id: int, snapshot_date: date
    ) -> Optional[Dict]:
        """getproject角色分布"""
        with self.get_connection() as conn:
            ...
            with conn.cursor() as cur:
                ...
                """,
                    (
                        ...
                        year_month,
                        project_id,
                        workload.get("total_issues", 0),
                        workload.get("total_journals", 0),
                        workload.get("as_manager", 0),
                        workload.get("as_implementation", 0),
                        workload.get("as_developer", 0),
                        workload.get("as_tester", 0),
                        workload.get("resolved_issues", 0),
                        workload.get("verified_issues", 0),
                    ),
                )

    def get_user_workload(
        self, user_id: int, year_month: str, project_id: Optional[int] = None
    ) -> List[Dict]:
